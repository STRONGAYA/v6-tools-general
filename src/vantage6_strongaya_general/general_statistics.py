...
def compute_aggregate_general_statistics(
    ...
    Returns:
        Dict[str, Union[str, None, List[Dict[str, Any]]]]: A dictionary containing the aggregated general statistics for categorical and numerical variables.
    """
    # Aggregate results
    ...
    for result in results:
        for result_, variables in result.items():
            if result_ == "categorical":
                ...
                numerical_df = numerical_df[~numerical_df["statistic"].isin(["mean", "std"])]
    ...
    if return_partials:
        ...
        return {"categorical_general_statistics": aggregate_categorical_df.to_json(),
...
def compute_aggregate_adjusted_deviation(
    ...
    Args:
        results_adjusted_deviation (List[Dict[str, Any]]): List of dictionaries containing the adjusted deviation results.
        ...
        for result_, variables in result.items():
            if result_ == "adjusted_deviation":
                ...
                if aggregate_deviation_df.empty:
                    ...
                    aggregate_deviation_df = pd.concat([aggregate_deviation_df, deviation_df], ignore_index=True)
    ...
    aggregate_deviation_df = pd.concat([aggregate_numerical_df, aggregate_deviation_df],
                                       ignore_index=True).sort_values('variable')

    # Add the adjusted deviation to the general statistics
    results_general_statistics.update({'numerical_general_statistics': aggregate_deviation_df.to_json()})

    if return_partials:
        safe_log("warn", "Returning partial aggregate-adjusted deviation statistics")
        results_general_statistics.update({'partial_deviation_results': results_adjusted_deviation})
...
def compute_local_adjusted_deviation(
        df: pd.DataFrame,
        numerical_aggregated_results: Optional[str] = None
) -> Dict[str, str]:
    """
    ...
    numerical_results = "{}" if numerical_aggregated_results is None else numerical_aggregated_results
    ...
    variables_to_analyse = [column_name for column_name in df.columns if
                            column_name in numerical_df["variable"].unique()]
    ...
    if not variables_to_analyse:
        ...
        safe_log("warn", "No variables to analyse for adjusted deviation due to lacking aggregate numerical statistics")
    else:
...
def _orchestrate_aggregate_adjusted_deviation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for variable in df["variable"].unique():
        ...
        column_statistics_series = column_statistics.set_index(['variable', 'statistic'])['value']
...
def _orchestrate_local_categorical_statistics(
    ...
    for column_name in df.columns:
        safe_log("info", f"General statistics for variable {column_name} are being computed")
...
def _orchestrate_local_numerical_statistics(
    ...
    for column_name in df.columns:
        ...
        # Get the inliers for the column from the provided dictionary
        if variable_details is not None and column_name in variable_details:
            inliers_range = variable_details[column_name].get("inliers", [float("-inf"), float("inf")])
        else:
        ...
        numerical_data.extend([
            ...
            (column_name, "std", np.sqrt(sum_errors2 / number_of_rows if number_of_rows > 0 else 1)),
...
def _orchestrate_local_adjusted_deviation(
    ...
    for column_name in df.columns:
        safe_log("info", f"Adjusted deviation for variable {column_name} is being computed")
        ...
        try:
            aggregated_mean = numerical_aggregated_results.loc[
                ...
                (numerical_aggregated_results["statistic"] == "mean"), "value"].values[0]
        except IndexError:
        ...
        adjusted_deviations.append((column_name, "adjusted_sum_of_squared_errors", adjusted_sum_of_squared_errors))
        adjusted_deviations.append((column_name, "count", number_of_rows))

    # Convert the list to a DataFrame
    adjusted_deviation_df = pd.DataFrame(adjusted_deviations, columns=["variable", "statistic", "value"])
...
def _compute_local_inliers_and_outliers(
    ...
    Args:
        ...
        inliers (List[Any]): A list of inliers for the categorical variable or a list of inliers range for numerical variables.
    ...
    elif isinstance(inliers, list) and len(inliers) == 2 and pd.api.types.is_numeric_dtype(column_values):
        # Numerical variable
        inliers_series = column_values[(column_values >= inliers[0]) & (column_values <= inliers[1])]
        outliers_series = column_values[(column_values < inliers[0]) | (column_values > inliers[1])]
    else:
        safe_log("warn", "Inliers are not in the correct format or are malformated for the expected datatype. "
...
def _compute_local_mean(column_values: pd.Series) -> Union[int, float]:
    ...
    rows = safe_calculate(_compute_local_number_of_rows, 1, column_values=column_values, drop_na=True)

    # Avoid division by zero
    return sum_val / rows if rows > 0 else 0.0


def _compute_local_min_max(column_values: pd.Series) -> Tuple[Union[int, float], Union[int, float]]:
    """
...
def _compute_local_number_of_rows(column_values: pd.Series, drop_na: bool = True) -> int:
    """
...
def _compute_local_quantile_sampling_variance(
    ...
    if n == 0:
        safe_log("warn", "Column contains no actual values, cannot compute quantile sampling variance")
        return 0.0
    ...
    try:
        quantiles = [np.quantile(np.random.choice(column_values, size=n, replace=True), quantile)
    ...
    if quantile_variance == 0:
        epsilon = 1e-10  # Very small number that will not meaningfully affect calculations
        safe_log("warn", f"Quantile sampling variance for {quantile} is 0, using epsilon value {str(epsilon)} instead")
...
def _compute_aggregate_summable_statistics(
        numerical_statistics: pd.Series,
        statistics_to_sum: List[str]
) -> pd.Series:
    """
    ...
    try:
        ...
        filtered_stats = numerical_statistics[
            numerical_statistics.index.get_level_values('statistic').isin(statistics_to_sum)]
        ...
        mask = ~numerical_statistics.index.get_level_values('statistic').isin(statistics_to_sum)
        result = pd.concat([numerical_statistics[mask], summed_stats])

        return result
    except Exception as e:
        safe_log("warn", f"Error computing aggregate summable statistics: {type(e).__name__}")
        return numerical_statistics
...
def _compute_aggregate_minmax(numerical_statistics: pd.Series) -> Dict[str, float]:
    ...
    Args:
        numerical_statistics (pd.Series): Series containing minimum and maximum values for a column per participating organisation.
    ...
    try:
        minimum = numerical_statistics[numerical_statistics.index.get_level_values('statistic') == 'min'].min()
        maximum = numerical_statistics[numerical_statistics.index.get_level_values('statistic') == 'max'].max()
...
def _compute_aggregate_mean(numerical_statistics: pd.Series) -> float:
    ...
    try:
        variable_name = numerical_statistics.index.get_level_values('variable').unique()[0]
...
def _compute_aggregate_deviation(numerical_statistics: pd.Series) -> float:
    ...
    try:
        ...
        total_sq_dev_sum = numerical_statistics.loc[(variable_name, 'sq_dev_sum')].iloc[0]
        total_count = numerical_statistics.loc[(variable_name, 'count')].iloc[0]

        return float(np.sqrt(total_sq_dev_sum / total_count)) if total_count > 0 else 0.0
    except Exception as e:
...
def _compute_aggregate_quantiles(numerical_statistics: pd.Series) -> Dict[str, float]:
    ...
    # Handle empty series safely
    if numerical_statistics.empty:
        return {'Q1': 0.0, 'Q2': 0.0, 'Q3': 0.0, 'Q1_std_err': 0.0, 'Q2_std_err': 0.0, 'Q3_std_err': 0.0}
    ...
    try:
        for i in range(1, 4):
            quantiles_i = numerical_statistics[
                numerical_statistics.index.get_level_values('statistic') == f'Q{i}'].values
            variances_i = numerical_statistics[
                numerical_statistics.index.get_level_values('statistic') == f'variance_Q{i}'].values
            ...
            # Using DerSimonian and Laird method to estimate tau2, equation 8 in https://doi.org/10.1016/j.cct.2006.04.004
            k = len(quantiles_i)
            omega_i0 = 1. / np.power(variances_i, 2)
            quantile_0 = np.sum(omega_i0 * quantiles_i) / np.sum(omega_i0)
            tau2_nom = np.sum(omega_i0 * np.power((quantiles_i - quantile_0), 2)) - (k - 1)
            tau2_den = np.sum(omega_i0) - np.sum(np.power(omega_i0, 2)) / np.sum(omega_i0)
            ...
            aggregate_quantile_std_err = np.sqrt(1. / np.sum(omega_i))
...
def _compute_aggregate_adjusted_deviation(numerical_statistics: pd.Series) -> float:
    ...
    try:
        local_adjusted_sum_of_squared_errors = numerical_statistics[
            numerical_statistics.index.get_level_values('statistic') == f'adjusted_sum_of_squared_errors'].values
        local_number_of_rows = numerical_statistics[
            numerical_statistics.index.get_level_values('statistic') == f'count'].values

        if len(local_adjusted_sum_of_squared_errors) == 0 or len(local_number_of_rows) == 0 or np.sum(
                local_number_of_rows) == 0:
            return 0.0

        aggregate_deviation = np.sqrt(np.sum(local_adjusted_sum_of_squared_errors) / np.sum(local_number_of_rows))
        return float(aggregate_deviation)
    except Exception as e:
        safe_log("warn", f"Error computing aggregate adjusted deviation: {type(e).__name__}")
        return 0.0

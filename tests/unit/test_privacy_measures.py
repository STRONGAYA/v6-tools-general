...
class TestMaskUnnecessaryVariables:
    ...
    def test_basic_variable_masking(self, mixed_data_sample):
        ...
        for var in variables_to_keep:
            pd.testing.assert_series_equal(
                result[var], mixed_data_sample[var], check_names=True
            )
    ...
    def test_mask_mixed_existing_nonexisting_variables(self, mixed_data_sample):
        ...
        try:
            ...
            existing_vars = [
                var for var in variables_to_keep if var in mixed_data_sample.columns
            ]
...
class TestApplySampleSizeThreshold:
    ...
    def test_threshold_with_sufficient_samples(self, mock_algorithm_client):
        ...
        with patch(
            "vantage6_strongaya_general.privacy_measures.get_env_var"
        ) as mock_env:
            mock_env.return_value = "10"  # Threshold of 10

            result = apply_sample_size_threshold(
                mock_algorithm_client, sufficient_data, variables_config
            )
    ...
    def test_threshold_with_insufficient_samples(self, mock_algorithm_client):
        ...
        with patch(
            ...
            try:
                result = apply_sample_size_threshold(
                    mock_algorithm_client, insufficient_data, variables_config
                )
    ...
    def test_threshold_with_zero_samples(self, mock_algorithm_client):
        ...
        empty_data = pd.DataFrame({"age": [], "gender": [], "organization_id": []})
        ...
        with patch(
            ...
            try:
                result = apply_sample_size_threshold(
                    mock_algorithm_client, empty_data, variables_config
                )
                assert len(result) == 0
            except Exception:
    ...
    def test_threshold_with_custom_threshold(self, mock_algorithm_client):
        ...
        test_data = pd.DataFrame({"value": range(50), "organization_id": [1] * 50})

        variables_config = {"value": {"datatype": "int", "inliers": [0, 100]}}
        ...
        with patch(
            ...
            result_low = apply_sample_size_threshold(
                mock_algorithm_client, test_data, variables_config
            )
    ...
    def test_threshold_with_invalid_threshold_value(self, mock_algorithm_client):
        ...
        test_data = pd.DataFrame({"value": [1, 2, 3, 4, 5], "organization_id": [1] * 5})

        variables_config = {"value": {"datatype": "int", "inliers": [0, 10]}}
...
class TestApplyDifferentialPrivacy:
    ...
    def test_basic_differential_privacy_mean(self):
        ...
        test_data = pd.DataFrame(
            {"value": [10, 20, 30, 40, 50], "organization_id": [1] * 5}
        )

        variables_config = {"value": {"datatype": "float", "inliers": [0, 100]}}

        result = apply_differential_privacy(
            test_data, variables_config, epsilon=1.0, return_type="dataframe"
        )
    ...
    def test_differential_privacy_with_different_epsilon(self):
        ...
        test_data = pd.DataFrame(
            {"value": [100] * 10, "organization_id": [1] * 10}  # Identical values
        )

        variables_config = {"value": {"datatype": "float", "inliers": [0, 200]}}

        # Test with high epsilon (less noise)
        result_high_epsilon = apply_differential_privacy(
            test_data, variables_config, epsilon=10.0, return_type="dataframe"
        )

        # Test with low epsilon (more noise)
        result_low_epsilon = apply_differential_privacy(
            test_data, variables_config, epsilon=0.1, return_type="dataframe"
        )
    ...
    def test_differential_privacy_verbose_return(self):
        ...
        variables_config = {"value": {"datatype": "float", "inliers": [0, 10]}}

        result = apply_differential_privacy(
            test_data, variables_config, epsilon=1.0, return_type="verbose"
        )
    ...
    def test_differential_privacy_with_multiple_variables(self):
        ...
        test_data = pd.DataFrame(
            {"var1": [10, 20, 30], "var2": [100, 200, 300], "organization_id": [1] * 3}
        )
    ...
    def test_differential_privacy_with_empty_dataframe(self):
        ...
        try:
            result = apply_differential_privacy(
                empty_data, variables_config, epsilon=1.0, return_type="dataframe"
            )
            assert isinstance(result, pd.DataFrame)
            assert len(result) == 0
        except Exception:
    ...
    def test_differential_privacy_edge_cases(self):
        ...
        zero_data = pd.DataFrame({"value": [0, 0, 0, 0, 0], "organization_id": [1] * 5})

        variables_config = {"value": {"datatype": "float", "inliers": [-10, 10]}}

        result_zero = apply_differential_privacy(
            zero_data, variables_config, epsilon=1.0, return_type="dataframe"
        )
        ...
        negative_data = pd.DataFrame(
            {"value": [-10, -5, -1, -3, -7], "organization_id": [1] * 5}
        )

        result_negative = apply_differential_privacy(
            negative_data, variables_config, epsilon=1.0, return_type="dataframe"
        )
    ...
    def test_differential_privacy_preserves_non_target_columns(self):
        ...
        variables_config = {"target_var": {"datatype": "float", "inliers": [0, 10]}}
        ...
        if "preserve_var" in result.columns:
            pd.testing.assert_series_equal(
                result["preserve_var"], test_data["preserve_var"]
            )

        if "organization_id" in result.columns:
            pd.testing.assert_series_equal(
                result["organization_id"], test_data["organization_id"]
            )
...
class TestPrivacyMeasuresIntegration:
    ...
    def test_complete_privacy_pipeline(self, mixed_data_sample, mock_algorithm_client):
        ...
        with patch(
            ...
            try:
                thresholded_data = apply_sample_size_threshold(
                    mock_algorithm_client, mixed_data_sample, variables_config
                )
            except Exception:
        ...
        try:
            final_data = apply_differential_privacy(
                masked_data, dp_variables_config, epsilon=1.0, return_type="dataframe"
            )
    ...
    def test_privacy_measures_with_edge_cases(
        self, edge_case_data, mock_algorithm_client
    ):
        ...
        for scenario_name, test_data in edge_case_data.items():
            ...
            if "organization_id" in test_data.columns:
                ...
                for col in test_data.columns:
                    if col != "organization_id" and pd.api.types.is_numeric_dtype(
                        test_data[col]
                    ):
                        variables_config[col] = {
                            "datatype": "float",
                            "inliers": [0, 1000],
                        }
    ...
    def test_privacy_measures_data_integrity(self, sample_numerical_data):
        ...
        masked_data = mask_unnecessary_variables(
            sample_numerical_data, variables_to_keep
        )
        ...
        try:
            dp_data = apply_differential_privacy(
                masked_data, dp_config, epsilon=1.0, return_type="dataframe"
            )

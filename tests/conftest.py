    ...
    data = {
        ...
        "outlier_variable": np.concatenate(
            [
                ...
                np.array(
                    [1000, -1000, 999, -999, 998, -998, 997, -997, 996, -996]
                ),  # Outliers
            ]
        ),
    }
...
def sample_categorical_data():
    ...
    data = {
        "gender": np.random.choice(
            ["Male", "Female", "Other"], n_samples, p=[0.48, 0.48, 0.04]
        ),
        "treatment_group": np.random.choice(
            ["Control", "Treatment_A", "Treatment_B"], n_samples, p=[0.3, 0.35, 0.35]
        ),
        "severity": np.random.choice(
            ["Mild", "Moderate", "Severe"], n_samples, p=[0.4, 0.4, 0.2]
        ),
...
def edge_case_data():
    ...
    # Single row dataset
    datasets["single_row"] = pd.DataFrame({"value": [42], "category": ["A"]}).astype(
        {"value": "Int64", "category": "category"}
    )

    # Single column dataset
    datasets["single_column"] = pd.DataFrame({"value": [1, 2, 3, 4, 5]}).astype(
        {"value": "Int64"}
    )
    ...
    datasets["extreme_values"] = pd.DataFrame(
        ...
        {"tiny": "Float64", "huge": "Float64", "zero": "Int64", "negative": "Float64"}
    )

    # Dataset with duplicate values
    datasets["duplicates"] = pd.DataFrame({"repeated": [5] * 100}).astype(
        {"repeated": "Int64"}
    )
...
def quantile_test_data():
    ...
    datasets["known_quantiles"] = pd.DataFrame(
        {"value": list(range(1, 101)), "organisation_id": [1] * 100}  # 1 to 100
    )
...
def assert_dataframes_equal(
    df1: pd.DataFrame, df2: pd.DataFrame, check_dtype: bool = True, rtol: float = 1e-5
):
    """
...
def create_federated_results(
    local_results: List[Dict], organization_ids: List[int]
) -> List[Dict]:
    """
    ...
    for i, (result, org_id) in enumerate(zip(local_results, organization_ids)):
        federated_results.append(
            {"organization_id": org_id, "result": result, "status": "success"}
        )
